(() => {
  ...
  // Patch approval: adminManager opsional di form PIC
  ...
  // Patch approval: tampilkan tag "opsional" + hint untuk Admin Manager
  function renderSignatures() {
    $("#signatureContainer").innerHTML = signatureRoles.map(role => `
      <article class="signature-card" data-signature-role="${role.id}">
        <h3>${escapeHtml(role.label)}${role.optional ? ' <span class="optional-tag">opsional</span>' : ''}</h3>
  ...
  // Patch approval: role optional tidak divalidasi
  function validate(data, { requireSignatures = true } = {}) {
    clearInvalidStates();
    const errors = [];

    [["executionDate", data.executionDate, `${FORM.dateLabel} wajib diisi.`],
     ["executionTime", data.executionTime, `${FORM.timeLabel} wajib diisi.`],
     ["picName", data.picName, `${FORM.picLabel} wajib diisi.`]].forEach(([id, value, message]) => {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Patch approval: bikin link untuk manajer
      if (result.approvalToken && result.submissionId) {
        const base = location.pathname.replace(/[^/]*$/, "");
        const approvalUrl = `${location.origin}${base}admin.html?approve=${encodeURIComponent(result.submissionId)}&token=${encodeURIComponent(result.approvalToken)}&type=${FORM_TYPE}`;
        try { await navigator.clipboard.writeText(approvalUrl); } catch (_) {}
        showToast("Checklist terkirim. Link approval sudah disalin.", "success", 6500);
      ...
      showToast(`Koneksi/backend gagal. Data diamankan di antrean perangkat. ${error.message}`, "error", 7000);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  function showShareApproval(url, submissionId) {
    ...
    banner.innerHTML = `
      <div>
        ...
        <p>Bagikan link ini ke manajer. Tidak perlu PIN admin.</p>
  ...
  async function exportPdf(inputData, options = {}) {
    const data = inputData && inputData.tasks ? inputData : collectData();
    if (!options.silentValidation) {
      if (!data.executionDate || !data.picName) {
        showToast("Isi tanggal dan PIC sebelum export PDF.", "error");
        return;
      }
    }
    ...
    signatureRoles.forEach((role, index) => {
      ...
      const sig = (data.signatures && data.signatures[role.id]) || {};
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.4);
      doc.text(sig.name || (role.optional ? "Menunggu approval" : "Belum diisi"), x + cardWidth / 2, y + 23, { align: "center", maxWidth: cardWidth - 3 });
      doc.setFont("helvetica", "bold");
  ...
  function exportWithPrintDialog(data) {
    ...
    const signatureCards = signatureRoles.map(role => {
      const signature = (data.signatures && data.signatures[role.id]) || {};
      ...
      return `
        <div class="signature-card-print">
          ${image}
          <div class="signature-name-print">${escapeHtml(signature.name || (role.optional ? "Menunggu approval manajer" : "Belum diisi"))}</div>
    ...
    const reportHtml = `<!doctype html>
      ...
      <body>
        ...
        <div class="meta">
          ...
          <div><strong>Status:</strong> ${escapeHtml((data.summary && data.summary.overallStatus) || "-")}</div>
        </div>
        <table>
  ...
  function setSubmitting(value) {
    ...
    if (label) label.textContent = value ? "Mengirim..." : "Kirim checklist";
  }
